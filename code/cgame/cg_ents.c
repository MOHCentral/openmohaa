...
void CG_AddPacketEntities(void)
{
    ...
    // Snapshot diagnostics: verify whether actor entities/models are present
    // in cg.snap before any render submission/filtering occurs.
    {
        static int s_snap_diag_frame = 0;
        s_snap_diag_frame++;
        if ((s_snap_diag_frame % 300) == 1) {
            int n_player = 0, n_modelanim = 0, n_vehicle = 0, n_other = 0;
            int u_count = 0;
            int u_modelidx[32];
            int u_count_by_idx[32];
            int ui;
            for (ui = 0; ui < 32; ui++) {
                u_modelidx[ui] = 0;
                u_count_by_idx[ui] = 0;
            ...
            for (num = 0; num < cg.snap->numEntities; num++) {
                ...
                if (es->eType == ET_PLAYER) {
                    n_player++;
                    track = 1;
                } else if (es->eType == ET_MODELANIM || es->eType == ET_MODELANIM_SKEL) {
                    n_modelanim++;
                    track = 1;
                } else if (es->eType == ET_VEHICLE) {
                    n_vehicle++;
                    track = 1;
                } else {
                    n_other++;
                ...
                if (track && es->modelindex > 0) {
                    for (ui = 0; ui < u_count; ui++) {
                        if (u_modelidx[ui] == es->modelindex) {
                            u_count_by_idx[ui]++;
                            break;
                        }
                    }
                    if (ui == u_count && u_count < 32) {
                        u_modelidx[u_count] = es->modelindex;
                        u_count_by_idx[u_count] = 1;
                        u_count++;
                    }
                }
            }
            ...
            cgi.Printf(
                "[MoHAA][CG-SNAP-SUM] ents=%d player=%d modelanim=%d vehicle=%d other=%d\n",
                cg.snap->numEntities,
                n_player,
                n_modelanim,
                n_vehicle,
            ...
            for (ui = 0; ui < u_count && ui < 12; ui++) {
                const char *mn = CG_ConfigString(CS_MODELS + u_modelidx[ui]);
                cgi.Printf(
                    "[MoHAA][CG-SNAP-MODEL] modelindex=%d count=%d name=%s\n",
                    u_modelidx[ui],
                    u_count_by_idx[ui],
                    (mn && mn[0]) ? mn : "(empty)"
                );
